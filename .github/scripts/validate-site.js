...
// ------------------------------------------- 9. 別言語の文字の混入（日本語サイト）
// 生成・編集の過程でハングルやキリル文字が1文字だけ紛れ込むことが実際に起きた
// （「無理」が「무理」、「悪化」が「악化」）。読者には文字化けにしか見えず、
// 1文字なので目視レビューでもまず気づかない。
// 外部ライブラリは各国語のデータを持っていて当然なので対象から外す。
const FOREIGN = [
  ['ハングル', /[가-힯ᄀ-ᇿ㄰-㆏]/g],
  ['キリル', /[Ѐ-ӿ]/g],
];
for (const f of [...htmlFiles, ...jsFiles]) {
  const r = rel(f);
  if (/\.min\.js$/.test(r) || /(^|\/)lib\//.test(r)) continue;
  const src = fs.readFileSync(f, 'utf8');
  for (const [name, re] of FOREIGN) {
    const m = src.match(re);
    if (m) {
      const chars = [...new Set(m)].slice(0, 8).join(' ');
      err('foreign-char', r, `${name}が混入しています: ${chars}`, '日本語に直してください。生成時に紛れ込んだ1文字であることが多く、読者には文字化けとして見えます。');
    }
  }
}

// --------------------------------------- 10. アフィリエイト広告のPR表記（法令）
// 2023年10月の景品表示法の指定告示（通称ステマ規制）により、広告であることを
// 消費者が判別できる表示が必要になった。AdSenseのポリシーではなく法令の話で、
// 表記漏れは事業者側の責任になる。バナーを足すときに付け忘れやすいので機械で見る。
for (const f of htmlFiles) {
  const src = fs.readFileSync(f, 'utf8');
  if (!/a8\.net|a8mat=/.test(src)) continue;
  // 「PR」「広告」「プロモーション」等、広告と分かる表示があるか
  if (!/affiliate-label|>\s*PR\s*<|アフィリエイト広告|プロモーションが含|広告を利用/.test(src)) {
    err('pr-label', rel(f), 'アフィリエイトリンクがあるのにPR表記がありません', '広告であることが分かる表示（「PR」等）を広告の近くに入れてください。景品表示法で求められています。');
  }
}
